class Main {
    ...
    handleEvent(ev) {
        let objetoEv = ev.target;
        let tipoEvento = ev.type;
        let tipoElemento = objetoEv.type;
        ...
        xhr.onreadystatechange = () => {
            if (xhr.readyState == 4) {
                if (xhr.status == 200) { /*la solicitud tuvo éxito*/
                    ...
                    let listaDis = JSON.parse(xhr.responseText); //listaDis guarda un array de Device (definido en device.ts)
                    for (let disp of listaDis) {
                        let icono; //este swich selecciona que ícono se va a mostrar según el tipo de dispositivo y lo guarda en icono
